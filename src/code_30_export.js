...
function saveWeekPDFs(grade=null,sht=null,week ){
  ...
  for (let j=0;j<shtArr.length;j++){
    ...
    for (let i=0;i<gradeArr.length;i++){
      ...
      sheetLink.getRange(2+i,1,1,1).setValue(dept);
      sheetLink.getRange(2+i,2,1,1).setValue(dateTxt2);
      sheetLink.getRange(2+i,3,1,1).setValue(secTxt);
      sheetLink.getRange(2+i,4,1,1).setValue(pdfUrl);
